...
export async function GET(req:Request){const s=supabase(req);try{await requireUser(s.client);const {data,error}=await s.client.rpc('pi_read');check(error);return s.finish(json({...data,server_now:Date.now()}))}catch(e){return s.finish(failure(e))}}
