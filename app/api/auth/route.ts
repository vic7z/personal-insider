...
export async function GET(req:Request){const s=supabase(req);try{const {data}=await s.client.auth.getUser();if(!data.user)return s.finish(json({user:null,setup:false,setupEmail:null}));const {data:profile,error}=await s.client.rpc('pi_profile');if(error){await s.client.auth.signOut();return s.finish(json({user:null,setup:false,setupEmail:null,notice:'Your account does not have active workspace access.'}))}return s.finish(json({user:profile,setup:false,setupEmail:null}))}catch(e){return s.finish(failure(e))}}
...
if(action==='register'){const name=z.string().trim().min(1).max(100).parse(b.name);const invite=z.string().regex(/^[a-f0-9]{64}$/,'Enter the invitation code from your admin').parse(b.invite?.trim());const {data,error}=await s.client.auth.signUp({email,password,options:{data:{name,invitation_code:invite},emailRedirectTo:new URL('/auth/callback',req.url).toString()}});check(error);return s.finish(json({ok:true,message:data.session?'Account created. You can now sign in.':'Check your email to confirm your account, then sign in.'}))}
const {error}=await s.client.auth.signInWithPassword({email,password});if(error)throw new HttpError(401,error.message);
const {error:access}=await s.client.rpc('pi_profile');if(access){await s.client.auth.signOut();throw new HttpError(403,'Your account does not have active workspace access')}
