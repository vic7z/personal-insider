alter table pi_private.guests add column feedback text not null default '' check(length(feedback)<=6000);
alter table pi_private.guests add column epic_status text not null default 'Pending' check(epic_status in ('Pending','Done'));
alter table pi_private.guests alter column meal_plan set default 'Breakfast only';
update pi_private.guests set meal_plan='Breakfast only',version=version+1 where meal_plan='Breakfast';
...
create or replace function pi_private.read_data() returns jsonb language plpgsql security definer set search_path='' as $$
declare m pi_private.members;gs jsonb;ms jsonb;hs jsonb;
begin
 m:=pi_private.member_context();
 select coalesce(jsonb_agg(case when m.role='Butler' then to_jsonb(g)||'{"notes":"","feedback":"","epic_status":"Pending","epic":"","travel_agent":"","celebration":"None","membership":"None","meal_plan":""}'::jsonb else to_jsonb(g) end order by g.departure,g.room),'[]'::jsonb) into gs from pi_private.guests g;
